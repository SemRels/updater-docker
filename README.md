...
```bash
docker pull ghcr.io/semrels/updater-docker:latest
```

Images are signed with [cosign](https://github.com/sigstore/cosign) and include a full SBOM attestation. Verify the signature:

```bash
cosign verify ghcr.io/semrels/updater-docker:latest \
  --certificate-identity-regexp 'https://github.com/SemRels/updater-docker/.github/workflows/release.yml.*' \
